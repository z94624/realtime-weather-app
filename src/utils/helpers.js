...
// locationName 是和 天氣預報 一樣的縣市名稱，而非 天氣觀測 的局屬氣象站名稱
export const getMoment = (locationName) => {
  ...
  // STEP 3：找不到的話則拋出錯誤訊息
  if (!location) {
    throw new Error(`找不到 ${locationName} 的日出日落資料`);
  }
...
/*
 * cityName: 縣市名稱，對應到天氣預報的地區名稱
 * locationName: 觀測站所在地區，對應到天氣觀測的地區名稱
 * sunriseCityName: 對應到日出日落時間的地區名稱
 */
